...
for i in range(1, N_EMPLOYEES + 1):
    ...
    # Attrition — higher for Sales and Junior
...
# ─────────────────────────────────────────────
# ONBOARDING TABLE (bottleneck simulation)
# ─────────────────────────────────────────────
onboarding = []
for _, emp in df_emp.iterrows():
    ...
    for step in ONBOARD_STEPS:
        # IT Setup is the bottleneck — takes much longer
        if step == "IT Setup":
...
conn.close()
print("\n✅ SQLite DB saved: output/hr_analytics.db")
